...
std::vector<std::string> findHours(odb::database& db, std::string username) {
	...
	query<user> q1( query<user>::name == username );	
	odb::result<user> r1=( db.query<user> (q1));
	for(odb::result<user>::iterator i=r1.begin();i!=r1.end();i++){
		query<review> q2( query<review>::user_id == (i->id) );
		odb::result<review> r2 (db.query<review> (q2));
		for(odb::result<review>::iterator j=r2.begin();j!=r2.end();j++){
			query<hours> q3( query<hours>::business_id == (j->business_id) );
			odb::result<hours> r3 (db.query<hours> (q3));
			for(odb::result<hours>::iterator k=r3.begin();k!=r3.end();k++){
				result.push_back(k->hours_);
			}	
		}
	}
...
std::vector<StarCount> countStars(odb::database& db, float latMin, float latMax, float longMin,float longMax) {
	std::vector<StarCount> result;
	transaction t(db.begin());
	// Your implementation goes here:
	// db.query<StarCount>("select ...")
	odb::result<StarCount> r = (db.query<StarCount> ("select count(review.id),review.stars from business join review on business.id=review.business_id where latitude>="+query<StarCount>::_val(latMin) +"and latitude<="+query<StarCount>::_val(latMax) +"and longitude>="+query<StarCount>::_val(longMin) +"and longitude<="+ query<StarCount>::_val(longMax)+" group by review.stars"));
	
	for(odb::result<StarCount>::iterator i=r.begin();i!=r.end();i++){
		result.push_back( StarCount( i->count,i->stars) );
	}
...
void createIndex(odb::database& db){
	...
	db.execute("CREATE NONCLUSTERED COLUMNSTORE INDEX b_simple ON business(id,longitude,latitude) ");
	db.execute("CREATE NONCLUSTERED COLUMNSTORE INDEX r_simple ON review(id,stars,business_id) ");
...
void dropIndex(odb::database& db){
	...
	db.execute("DROP INDEX b_simple ON business");
	db.execute("DROP INDEX r_simple ON review");
